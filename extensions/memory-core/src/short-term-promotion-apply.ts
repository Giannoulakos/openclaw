...
import { buildBudgetedMemoryAppend, canMemoryAppendEverFit } from "./memory-budget-append.js";
...
export async function applyShortTermPromotions(
  ...
  const appendSection = buildPromotionSection(
    toAppend,
    nowMs,
    options.timezone,
    options.maxPromotedSnippetTokens,
  );
  const appendCanPossiblyFit = canMemoryAppendEverFit({
    existingMemory,
    newSection: appendSection,
    budgetChars,
  });
